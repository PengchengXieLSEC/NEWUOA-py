...
def NEWUOAStep5():
    ...
    if t >= 0:
        ...
        et[t] = 1
        eHw = et - Hw
        H = H + (alpha * (eHw.reshape(n + m + 1, 1) * eHw) - beta * H[:, t].reshape(n + m + 1, 1) * H[t, :] + tau * (H[:, t].reshape(n + m + 1, 1) * eHw + eHw.reshape(n + m + 1, 1) * H[t, :])) / sigma
        if (RATIO >= 0.5):
            C = Fnew - Qnew
            Lcg = C * H[:, t]
        else:
...
def NEWUOAStep9():
    ...
    def eta(u):  # u为array类型
        return H[0:m, t] * dot(XX0.T, u)#5*1   5*1
...
def NEWUOAStep10():
    ...
    M1 = max(NORMD, delta)
    if ((M1 - rho) / rho < 10 ** -6) and (RATIO <= 0):
        NEWUOAStep11()
    else:
        NEWUOAStep2()
...
rhobeg = 1.0
rhoend = 10 ** -6
Fopt,Xopt = NEWUOAMethod(F, M, N, xbeg, rhobeg, rhoend)
